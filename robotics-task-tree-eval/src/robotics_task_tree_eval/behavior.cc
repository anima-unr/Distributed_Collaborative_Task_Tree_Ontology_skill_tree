...
std::vector<std::string> listOfSkill = {"Tea", "Sandwich"};
...
Behavior::Behavior(NodeId_t name, NodeList peers, NodeList children,
    ...
    boost::posix_time::millisec mtime) : Node(name,
      ...
      skill_choose_sub_ = local_.subscribe("/skill_choose", 1000, &Behavior::SkillChooseCallback, this );
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
// Skill Choose BEHAVIOR
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
SkillChooseBehavior::SkillChooseBehavior() {}
SkillChooseBehavior::SkillChooseBehavior(NodeId_t name, NodeList peers, NodeList children,
    NodeId_t parent,
    State_t state,
    std::string object,
    bool use_local_callback_queue,
  ...
  // Initialize activation queue
       printf("SkillChooseBehavior::SkillChooseBehavior WAS CALLED\n");
...
void SkillChooseBehavior::UpdateActivationPotential() {
  //ROS_INFO("SkillChooseBehavior::UpdateActivationPotential was called!!!!\n");
  ...
  for (NodeListPtrIterator it = children_.begin();
      it != children_.end(); ++it) {
    //ROS_INFO( "\t[%s]: checking [%d]: [%d|%d|%d|%d]\n\n\n\n\n\n", name_->topic.c_str(), (*it)->state.owner.node, (*it)->state.done, (*it)->state.peer_done, (*it)->state.active, (*it)->state.peer_active );
    // find the first not done/active node
    if( !(*it)->state.done && !(*it)->state.peer_done && !(*it)->state.peer_active && (*it)->state.owner.skill==skill_ )
    {
      //ROS_INFO("check inside %f",(*it)->state.activation_potential);
...
bool SkillChooseBehavior::Precondition() {
    // ROS_INFO("SkillChooseBehavior::Precondition was called!!!!\n");
  bool satisfied = true;
  for (NodeListPtrIterator it = children_.begin();
      it != children_.end(); ++it) {
...
uint32_t SkillChooseBehavior::SpreadActivation() {
  ROS_DEBUG("SkillChooseBehavior::SpreadActivation was called!!!!");
  ...
  for (NodeListPtrIterator it = children_.begin(); it != children_.end();
      ++it) {
    if((*it)->mask.skill==skill_)
    SendToChild((*it)->mask, msg);
  }
}

bool SkillChooseBehavior::IsDone() {
  ROS_DEBUG("[%s]: SkillChooseBehavior::IsDone was called", name_->topic.c_str() );
...
void Behavior::SkillChooseCallback( std_msgs::String msg)
  {
    // std::vector<std::string>::iterator itr = std::find(listOfSkill.begin(), listOfSkill.end(), msg.data);
    skill_ = distance(listOfSkill.begin(), find(listOfSkill.begin(), listOfSkill.end(), msg.data)) + 1;
    ROS_WARN("%d\n\n\n\n\n\n\n\n\n\n\n\n\n\n",skill_);

// if (itr != end(listOfSkill)) {
//   cout << "Element " << msg.data << " is present at index " << distance(listOfSkill, itr) << " in the given array";
//   skill_ = distance(listOfSkill, itr);
// }
// else {
