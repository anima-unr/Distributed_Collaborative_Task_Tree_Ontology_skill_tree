...
void Node::InitializeBitmask(NodeId_t * node) {
     ROS_INFO("Node::InitializeBitmask was called!!!!2 %s\n",node->topic.c_str());
  node->mask = GetBitmask(node->topic);
}

void Node::InitializeBitmasks(NodeListPtr nodes) {
     ROS_INFO("Node::InitializeBitmasks was called!!!!1 \n");
...
void Node::ReceiveFromChildren(ConstControlMessagePtr_t msg) {
  ...
  child->state.highest.skill = msg->highest.skill;
...
void Node::NodeInit(boost::posix_time::millisec mtime) {
  ...
  std::string filename = "/home/bashira/catkin_ws_old/src/Distributed_Collaborative_Task_Tree/Data/" + name_->topic + "_Data_.csv";
...
void Node::PublishStatus() {
  ...
  msg.owner.skill = state_.owner.skill;
  msg.active = state_.active;
  msg.done = state_.done;
  msg.activation_level = state_.activation_level;
  msg.activation_potential = state_.activation_potential;
  ...
  msg.highest.skill = state_.highest.skill;
...
NodeBitmask Node::GetBitmask(std::string name) {
     ROS_INFO("Node::GetBitmask was called!!!!%s\n",name.c_str());
  // Split underscores
  ...
  mask.skill  = static_cast<uint8_t>(atoi(split_vec[4].c_str()));
  ROS_INFO("Node::GetBitmask was called!!!!end\n");
  return mask;
}
