...
class Behavior: public Node {
 public:
  ...
  virtual ~Behavior();
  void SkillChooseCallback( std_msgs::String msg);
static task_net::hold_status hold_status_dummy1_;
ros::Subscriber skill_choose_sub_;
uint skill_;
 private:
};
class SkillChooseBehavior: public Behavior {
 public:
  SkillChooseBehavior();
  SkillChooseBehavior(NodeId_t name, NodeList peers, NodeList children,
    NodeId_t parent,
    State_t state,
    std::string object,
    bool use_local_callback_queue = false,
    boost::posix_time::millisec mtime = boost::posix_time::millisec(BEHAVIOR_SLEEP_TIME));
  virtual ~SkillChooseBehavior();
