...
class RemoteMutexService {
 public:
  ...
  explicit RemoteMutexService(const char* name)
      : record_object("/home/bashira/catkin_ws_old/src/Distributed_Collaborative_Task_Tree/Data/remote_mutex.csv",
        100) {
    locked = false;
    owner = "";
    ...
    ns.param<std::string>( "/topic", root_topic_, "SKILL_7_1_014_0_state");
    ...
    if (req.request) {
      ...
      if (locked) {
        ...
        //if( is_eq(GetBitmask(req.name), top_level_state_.highest) )
        {
          ...
          activation_potential = req.activation_potential;
          mut.unlock();
          ROS_INFO("activation_potential %f\n\n",activation_potential);
          if( activation_potential > 0.001 )
          {
            ROS_INFO("Mutex Locked - Granted Access: %s", req.name.c_str());
            mut.lock();
